...
class ExcelExporter {
  ...
  _createTableWorksheet(table) {
    ...
    table.fields.forEach(field => {
      const typeDisplay = typeof field.type === 'object' && field.type.type_name ? field.type.type_name : field.type;
      worksheet.addRow([field.name, typeDisplay, field.not_null ? '×' : '○', field.default || '', field.pk ? '○' : '', field.unique ? '○' : '', field.increment ? '○' : '', field.note || '']);
    });
  ...
  async _waitForFileCreation(outputPath) {
    ...
    if (!fs.existsSync(outputPath)) {
      ...
      const dirContent = dirExists ? fs.readdirSync(path.dirname(outputPath)) : [];
      throw new Error(`Excel file was not created after ${maxRetries} attempts: ${outputPath} (CI: ${isCI}). Directory exists: ${dirExists}, Directory content: ${JSON.stringify(dirContent)}`);
    }
    const stats = fs.statSync(outputPath);
    if (stats.size === 0) {
